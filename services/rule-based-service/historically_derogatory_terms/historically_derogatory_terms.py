...
from src.helpers import find_phrases_in_text, load_phrases_from_json_data
from src.stanza import StanzaProtocol

logger = logging.getLogger(__name__)

def analyse(text: str, language: str = "en", stanza: StanzaProtocol | None = None) -> list[BiasIndicatorResult]:
    language_map = {
        "en": "historically_derogatory_terms_en.json",
        "el": "historically_derogatory_terms_el_lemmas.json",
        "el_patterns": "historically_derogatory_terms_el_patterns.json",
        "el_fallback": "historically_derogatory_terms_el.json",
    }
    ...
    with open(os.path.join(os.path.dirname(__file__), filename), "r") as f:
        data = json.load(f)
        historically_derogatory_terms = load_phrases_from_json_data(data)

    found_phrases = []
    try:
        if language == "el" and stanza:
            ...
            found_phrases = stanza.find_phrases_in_text(text, list(historically_derogatory_terms), patterns)
        else:
            found_phrases = find_phrases_in_text(text, list(historically_derogatory_terms))
    ...
    return [
        BiasIndicatorResult(
            bias_indicator_key="historicallyDerogatoryTerms",
