...
def analyse(text: str, language: str = "en", morphodita: MorphoDiTaProtocol | None = None, omorfi: OmorfiProtocol | None = None, stanza: StanzaProtocol | None = None) -> list[BiasIndicatorResult]:
    language_map = {
        "en": "absolute_terms_en.json",
        "fi": "absolute_terms_fi_lemmas.json",
        "fi_fallback": "absolute_terms_fi.json",
        "pt": "absolute_terms_pt_patterns.json",
        "cs": "absolute_terms_cs_lemmas.json",
        "cs_fallback": "absolute_terms_cs.json",
        "el": "absolute_terms_el_lemmas.json",
        "el_patterns": "absolute_terms_el_patterns.json",
        "el_fallback": "absolute_terms_el.json",
    }
    ...
    with open(os.path.join(os.path.dirname(__file__), filename), "r") as f:
        data = json.load(f)
        absolute_terms = load_phrases_from_json_data(data)

    found_phrases = []
    try:
        if language == "cs" and morphodita:
            found_phrases = morphodita.find_phrases_in_text(text, list(absolute_terms))
        elif language == "pt":
            found_phrases = enhanced_find_phrases_in_text(text, list(absolute_terms))
        elif language == "fi" and omorfi:
            found_phrases = omorfi.find_phrases_in_text(text, list(absolute_terms))
            ...
            found_phrases = stanza.find_phrases_in_text(text, list(absolute_terms), patterns)
        else:
            found_phrases = find_phrases_in_text(text, list(absolute_terms))
