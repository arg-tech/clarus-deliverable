...
def analyse(text: str, language: str = "en", morphodita: MorphoDiTaProtocol | None = None, omorfi: OmorfiProtocol | None = None, stanza: StanzaProtocol | None = None) -> list[BiasIndicatorResult]:
    language_map = {
        "en": "intensifying_adverbs_en.json",
        "el": "intensifying_adverbs_el_lemmas.json",
        "el_patterns": "intensifying_adverbs_el_patterns.json",
        "el_fallback": "intensifying_adverbs_el.json",
        "fi": "intensifying_adverbs_fi_lemmas.json",
        "fi_fallback": "intensifying_adverbs_fi.json",
        "cs": "intensifying_adverbs_cs_lemmas.json",
        "cs_fallback": "intensifying_adverbs_cs.json",
        "pt": "intensifying_adverbs_pt_patterns.json",
    }
    ...
    with open(os.path.join(os.path.dirname(__file__), filename), "r") as f:
        data = json.load(f)
        intensifying_adverbs = load_phrases_from_json_data(data)

    found_phrases = []
    try:
        if language == "cs" and morphodita:
            found_phrases = morphodita.find_phrases_in_text(text, list(intensifying_adverbs))
        elif language == "pt":
            found_phrases = enhanced_find_phrases_in_text(text, list(intensifying_adverbs))
        elif language == "fi" and omorfi:
            found_phrases = omorfi.find_phrases_in_text(text, list(intensifying_adverbs))
            ...
            found_phrases = stanza.find_phrases_in_text(text, list(intensifying_adverbs), patterns)
        else:
            found_phrases = find_phrases_in_text(text, list(intensifying_adverbs))
    ...
    return [
        BiasIndicatorResult(
            bias_indicator_key="intensifyingAdverbs",
